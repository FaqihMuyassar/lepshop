      ...
      >
        ...
        <div className="h-1/2 flex flex-col justify-between md:pl-4 px-3 md:pt-4 pt-2">
          <div className="">
            ...
            <div>
              <p className="text-xs text-slate-400 md:pt-2 pt-1 dark:text-white">{Specification.All}</p>
            </div>
          </div>
          <div className="md:pt-8 pt-2 ">
            <h3 className="md:text-xl text-lg font-bold md:pb-2 pb-0 dark:text-white">{price}</h3>
          </div>
          <div className="flex flex-col justify-between">
            <div className="flex items-center gap-1">
            <FaLocationDot size={12}/>
              <p className="text-sm text-slate-400 dark:text-white">{location}</p>
            </div>
            <div className="flex">

            <div className="flex items-center gap-1 ">
            <FaStar size={12} color="#FFC700"/>
              <p className="text-sm text-slate-400 dark:text-white">{rating} | {sold}</p>
            </div>
          </div>
            </div>
        </div>
